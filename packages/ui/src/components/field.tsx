...
const Field = ({
  ...
  <div
    // oxlint-disable-next-line jsx-a11y/prefer-tag-over-role -- fieldset carries legacy min-width and flex quirks; shadcn's Field is a div with role=group
    role="group"
    data-slot="field"
...
const FieldError = ({
  ...
  const messages = [
    ...new Set(errors?.flatMap((fieldError) => (fieldError?.message ? [fieldError.message] : []))),
  ];
  const content =
    children ||
    (messages.length > 1 ? (
