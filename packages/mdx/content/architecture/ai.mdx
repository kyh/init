...
```typescript
// apps/nextjs/src/app/(dashboard)/dashboard/[teamSlug]/(home)/_components/ai-chat-form.tsx
"use client";
...
```typescript
// apps/nextjs/src/app/(dashboard)/dashboard/[teamSlug]/(home)/_components/ai-chat-message.tsx
...
```typescript
// apps/nextjs/src/app/(dashboard)/dashboard/[teamSlug]/(home)/_components/ai-chat-input.tsx
...
// In the API route
export async function POST(request: Request) {
  const { messages, teamSlug } = await request.json();
...
export default function TeamDashboard({ params }: { params: { teamSlug: string } }) {
  return (
    <div className="container mx-auto p-6">
      ...
      <div className="mt-8">
        ...
        <div className="h-96 border rounded-lg">
          <AIChatForm teamSlug={params.teamSlug} />
        </div>
      </div>
    </div>
  );
...
const result = streamText({
  model: openai("gpt-4o-mini"),
  system: `You are a helpful AI assistant for the ${teamSlug} team. 
  ...
  const contextualizedMessages = [
    {
      ...
      content: `You are assisting the ${teamSlug} team. Their recent activity includes...`
    },
    ...messages
  ];
...
export const chatMessages = pgTable("chat_messages", (t) => ({
  ...
  teamId: t.uuid().notNull().references(() => teams.id),
...
export async function POST(request: Request) {
  ...
  const team = await caller.team.getTeam({ slug: teamSlug });
...
export async function POST(request: Request) {
  const identifier = `team-${teamSlug}`;
