...
Initialize in a client provider (next to `ORPCReactProvider` in `apps/web/src/app/layout.tsx`), then identify against the entities Init already has - user and organization:
...
For events you can't trust the client to report, an oRPC middleware sees every procedure call:

```typescript
// packages/api/src/orpc.ts
const analyticsMiddleware = o.middleware(async ({ next, path, context }) => {
  const result = await next();
  // capture(context.session?.user.id, path.join("."))
  return result;
});
```
