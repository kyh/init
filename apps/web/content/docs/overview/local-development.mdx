...
`POSTGRES_URL` is the only database input - the local Docker container in dev, Vercel Postgres (or any Postgres) in production. `COMPOSE_PROJECT_NAME` namespaces that container's volume so two clones don't share one database; `pnpm bootstrap` writes it from the repo folder name.
