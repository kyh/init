...
Always `.enableRLS()` - authorization lives in oRPC, RLS blocks direct PostgREST access.
...
`organizationProcedure` (from `../orpc`) supplies the `slug` input and proves membership before the handler runs; procedures needing more input use `protectedProcedure.input(schema).use(requireOrganization)` instead. Register in `packages/api/src/root-router.ts`. See the [API guide](/docs/architecture/api) for the full pattern.
