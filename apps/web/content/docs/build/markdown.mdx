...
These docs are written for humans. Agents get their own: `.github/workflows/openwiki-update.yml` runs [OpenWiki](https://github.com/langchain-ai/openwiki) weekly, regenerating an agent wiki (`openwiki/`) plus `AGENTS.md` from the current code and opening a PR with the diff. Code drift becomes a reviewable docs PR instead of silent rot.

To enable: add an `ANTHROPIC_API_KEY` repo secret, set the `OPENWIKI_ENABLED` repo variable to `true` (or trigger the workflow manually once from the Actions tab), and merge the first PR.
