...
```typescript
export const example = pgTable.withRLS("example", (t) => ({
  // withRLS, always
...
- **snake_case is automatic** - `pgTable` in `drizzle-schema.ts` is `pgTableCreator((name) => name, "snake_case")`, so write `organizationId` and get `organization_id`
- **`pgTable.withRLS` for every table** - no policies, deny by default. Authorization lives in the oRPC procedures; the server connection bypasses RLS as table owner. It's the backstop: if anything ever reaches the database on a lower-privileged role or through a data API, an RLS-less table is world-readable.
...
Declare relations in `packages/db/src/drizzle-relations.ts`, one `defineRelations` graph over both schema files, so `db.query.*.findMany({ with })` works:
...
- Relations declared in `drizzle-relations.ts`
- `pnpm db:push` run
