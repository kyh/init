...
- **`.enableRLS()` on every table** - no policies, deny by default. Authorization lives in the oRPC procedures; the server connection bypasses RLS as table owner. It's the backstop: if anything ever reaches the database on a lower-privileged role or through a data API, an RLS-less table is world-readable.
