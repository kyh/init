...
- **`.enableRLS()` on every table** - no policies, deny by default. Authorization lives in the oRPC procedures; the server connection bypasses RLS as table owner. Forget this and the table is readable through PostgREST with the anon key on hosted Supabase.
