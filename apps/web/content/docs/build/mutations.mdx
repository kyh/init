...
Writes go through oRPC procedures: Zod validates, `requireOrganization` proves org membership, TanStack Query refreshes the cache. The todo list and waitlist form are the two reference implementations.
...
- **Membership before the handler** - `requireOrganization` (`packages/api/src/orpc.ts`) reads the `slug` off the validated input, resolves the org, and proves the caller is a member; the handler just reads `context.organization.id`
...
The same pattern covers mutations that wrap a better-auth call instead of an oRPC procedure:
...
3. **Error handling** - [oRPC error codes](/docs/architecture/api)
