...
You don't write invalidation code for the common case. The query client invalidates **everything** after any successful mutation:
...
Blunt, but always correct - every list refetches with fresh data. If a hot path refetches too much, narrow it per-mutation:
...
const createTodo = useMutation({
  ...
  onSuccess: () =>
    queryClient.invalidateQueries({
      queryKey: trpc.todo.list.queryOptions({ slug }).queryKey,
    }),
});
```

A per-mutation `onSuccess` replaces the global default - once you narrow, you own the invalidation.
