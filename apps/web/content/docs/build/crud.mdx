...
Org-scoped schemas extend `organizationInput`, so the `slug` is part of each procedure's declared contract - and `requireOrganization` (next step) will only compose onto a procedure whose input carries it.
...
`requireOrganization` (in `packages/api/src/orpc.ts`) resolves the org named by `input.slug` and proves membership before any handler runs, so handlers just read `context.organization.id`. `list` takes nothing but the slug, so it uses `organizationProcedure` - the prebuilt combination of the two. Every mutation still filters by `organizationId` in the `where` clause, so a valid member of org A can never touch org B's rows.
...
Routers test without a database. `createRouterClient` plus the mock context from `packages/api/src/test-utils.ts`:
...
const createCaller = (context: ReturnType<typeof createMockContext>) =>
  createRouterClient(postRouter, { context });
...
- Router applies `requireOrganization` and filters mutations by `organizationId`
