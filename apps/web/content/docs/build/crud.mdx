...
Every input carries the organization `slug` - the router uses it to verify membership.
...
Nothing else to do. The mobile app (`apps/mobile/src/trpc/`) consumes the same `AppRouter` type, so `trpc.post.list` works there too. The extension iframes the web app and gets the feature for free.

## Checklist

- Table has `organizationId` + index + `.enableRLS()`
- `pnpm db:push` ran
- Zod schema validates every input, including `slug`
- Router checks org membership and filters mutations by `organizationId`
