...
Every tRPC call emits one JSON line on the server (`packages/api/src/observability/logger.ts`):

```text
{"ts":"2026-08-05T12:04:31.882Z","requestId":"5f3c…","type":"mutation","path":"todo.create","durationMs":18,"ok":true,"userId":"usr_…"}
```
...
On Vercel these land in runtime logs as-is; any drain that understands JSON lines (Better Stack, Axiom, Datadog) can index them without a parser. The lines are silent under vitest so unit tests aren't drowned in them.
