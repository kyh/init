...
Query stats come from `pg_stat_statements` - enable it once (`CREATE EXTENSION IF NOT EXISTS pg_stat_statements;`), then read it directly or in your provider's dashboard (Neon surfaces it under Monitoring):
