...
Ship Init to production. The web app targets Vercel; the database targets Neon. Mobile, extension, and desktop each have their own pipeline.
...
```
# Database — Neon's pooled connection string (hostname contains -pooler)
...
The auth base URL derives from Vercel's built-in `VERCEL_ENV` / `VERCEL_PROJECT_PRODUCTION_URL` / `VERCEL_URL` - nothing to set. Adding the Neon integration to the Vercel project populates `POSTGRES_URL`, and creating a Blob store under Storage populates `BLOB_READ_WRITE_TOKEN`.
...
1. **Create a production project** at [neon.tech](https://neon.tech), or add the Neon integration from the Vercel dashboard
2. **Push the schema**:
...
Any Postgres works - Neon is the default because of branching (below), not because anything in the code depends on it.

RLS is already on for every table with no policies - deny by default. Nothing exposes Postgres directly, so this is belt-and-braces; authorization lives in tRPC, and the server connection bypasses RLS as table owner.

### Branching

Neon branches are copy-on-write, so a branch is a full database with production's data at a fraction of the storage. Two uses:
...
- **Agents and experiments** - `neonctl branches create --name agent/<task>` gives an isolated database to migrate against or destroy freely
...
Neon also keeps point-in-time restore over a retention window, and a branch taken before a migration is itself a cheap rollback target.
...
- `POSTGRES_URL` uses Neon's pooled endpoint (`-pooler` in the hostname) - the client is configured for it
