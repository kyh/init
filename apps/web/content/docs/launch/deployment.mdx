...
RLS is already on for every table with no policies - deny by default. Authorization lives in the oRPC procedures; the server connection bypasses RLS as table owner.
