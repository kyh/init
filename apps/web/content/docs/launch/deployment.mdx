...
3. **Create the avatars bucket** - a public `avatars` bucket with the policies from `packages/db/supabase/seed.sql`. Local dev gets this from `config.toml`; hosted projects need it created in the dashboard.
...
- Avatars bucket + policies exist in the production Supabase project
