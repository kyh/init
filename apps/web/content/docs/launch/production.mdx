...
2. **oRPC procedures** - the real authorization. `protectedProcedure` requires a session; `requireOrganization` (in `packages/api/src/orpc.ts`) resolves the org and proves membership before any org-scoped handler runs
