...
Neon (like plain Postgres) exposes no HTTP data API, so tRPC is the only way in and RLS is the backstop, not the gate. If you regenerate the auth schema, re-add `.enableRLS()` to every table.
