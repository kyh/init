...
1. **RLS deny-by-default** - every table is declared through `pgTable.withRLS` with no policies, so the anon key gets nothing through PostgREST
...
On a hosted Supabase project, also disable the Data API (Settings → API) - local dev already has it off in `packages/db/supabase/config.toml`. And if you regenerate the auth schema, declare every table through `pgTable.withRLS` again.
