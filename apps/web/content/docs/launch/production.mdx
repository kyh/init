...
- `POSTGRES_URL` should be the pooled endpoint; the client already sets `prepare: false` for it, and `drizzle.config.ts` prefers `POSTGRES_URL_NON_POOLING` so DDL runs on a direct connection
