...
- **`packages/api/src/auth/auth.ts`** - the `oAuthProxy` fallback `productionURL` is `init.kyh.io`; on Vercel it's derived from `VERCEL_PROJECT_PRODUCTION_URL`, but update the fallback anyway
...
- **Email delivery** - no provider is wired. Password reset silently can't send, and organization invites only surface in-app. Wire `emailAndPassword.sendResetPassword` and the org plugin's `sendInvitationEmail` in `packages/api/src/auth/auth.ts` to a provider (Resend, Postmark)
...
- [ ] `site-config.ts`, OAuth proxy fallback, `githubUrl`, `og.jpg` updated
- [ ] Unique `BETTER_AUTH_SECRET`; separate GitHub OAuth apps
- [ ] Production schema pushed; `avatars` bucket created; Data API disabled
- [ ] Email delivery wired (or password reset + invites consciously deferred)
