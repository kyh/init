...
Init uses Postgres with Drizzle ORM for type-safe queries - a Docker container locally, Neon in production. This guide covers the setup, schema, and development workflow.

## Overview

The stack:

- **Postgres** - `postgres:17-alpine` via Docker Compose locally, Neon in production
...
```bash
pnpm db:start    # docker compose up -d --wait (Postgres on :54322)
...
Port 54322 rather than 5432, so a Postgres already installed on the host doesn't clash. `--wait` blocks on the container's healthcheck, which checks over TCP - the throwaway server that runs during first-boot `initdb` answers on the Unix socket, so a socket check would return before the real server is listening.
...
`POSTGRES_URL` points at a transaction-mode pooler (Neon's `-pooler` endpoint) - the client must keep `prepare: false`.
