...
Every table calls `.enableRLS()` with no policies - deny by default. The public schema is reachable through PostgREST with the anon key, so this blocks direct access. Authorization lives in oRPC. The server's Drizzle connection is unaffected: the table owner bypasses RLS.
