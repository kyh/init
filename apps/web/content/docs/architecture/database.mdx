...
Config lives in `packages/db/supabase/config.toml`. Supabase Auth is disabled (better-auth owns auth). Storage is enabled with a public `avatars` bucket; its access policies live in `packages/db/supabase/seed.sql`.

```bash
pnpm db:start   # supabase start (Postgres on :54322, Studio on :54323)
```

For hosted projects, create the `avatars` bucket and policies in the Supabase dashboard.
