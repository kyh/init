...
Every table enables RLS with no policies, so the Data API serves zero rows to `anon`/`authenticated` — authorization lives in tRPC. Hosted projects should disable the Data API under Settings → API; nothing in the app uses it.
