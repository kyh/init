...
Even if the Data API is on, every table enables RLS with no policies (deny-by-default) — Supabase's own security model — so `anon`/`authenticated` get zero rows. New Supabase projects also stop auto-granting table access entirely; for older projects, disable the Data API once under Settings → API.
