...
export const waitlistRouter = createTRPCRouter({
  join: publicProcedure.input(z.object({ email: z.email() })).mutation(async ({ input, ctx }) => {
...
export const teamRouter = createTRPCRouter({
  ...
  // Create new team
  create: protectedProcedure.input(createTeamInput).mutation(async ({ input, ctx }) => {
...
```typescript
// Next.js (apps/web/src/trpc/react.tsx)
// Expo (apps/mobile/src/trpc/react.tsx)
import { createTRPCReact, createTRPCReact } from "@trpc/react-query";

import type { AppRouter, AppRouter } from "@repo/api";
...
export const postsRouter = createTRPCRouter({
  ...
  create: protectedProcedure.input(createPostInput).mutation(async ({ input, ctx }) => {
