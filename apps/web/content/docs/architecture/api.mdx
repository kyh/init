...
├── origin-guard.ts        # Cross-origin check for the RPC endpoint
...
Org-scoping is a middleware, `requireOrganization`. It reads `input.slug`, resolves the organization, and proves the caller is a member before the handler runs - so a handler cannot read or write another tenant's rows by forgetting a check:
...
// The common case: an org-scoped procedure whose only input is the slug
...
A procedure that needs more than the slug composes the pieces itself:

```typescript
protectedProcedure.input(createTodoInput).use(requireOrganization).handler(...)
```

The middleware declares its input as `{ slug: string }`, so TypeScript refuses to apply it to a procedure whose schema doesn't carry a slug. That is why org-scoped schemas extend `organizationInput` - the schema states the procedure's whole contract, and the guarantee is checked at compile time rather than assumed.
...
Auth cookies are `SameSite=None`, so the browser's own CSRF backstop is off for the RPC endpoint. `isUntrustedOrigin` (`packages/api/src/origin-guard.ts`) runs at the HTTP boundary in the route handler and rejects any request whose browser-supplied `Origin` / `Sec-Fetch-Site` isn't same-origin or allow-listed. Non-browser callers send neither header and pass through; session auth still applies.
...
import { protectedProcedure, requireOrganization } from "../orpc";
...
Routers ship with node:test tests next to the code (`todo-router.test.ts`). Shared helpers live in `packages/api/src/test-utils.ts`. Tests call procedures through `createRouterClient`, which runs the real middleware chain against a mock context:

```typescript
import { createRouterClient } from "@orpc/server";

const caller = createRouterClient(todoRouter, { context: createMockContext() });
...
- **Check org access** - Every multi-tenant procedure applies `requireOrganization`, so membership is proven before the handler runs
