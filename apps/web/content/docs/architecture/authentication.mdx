...
Organization membership is a layer on top: `organizationProcedure` in `packages/api/src/trpc.ts` takes an organization `slug`, resolves it, and proves the caller is a member before the handler runs - handlers get `ctx.organization` and `ctx.membership`.
