...
The oRPC context resolves the session from request headers:
...
Organization membership is a layer on top: `organizationProcedure(schema)` in `packages/api/src/orpc.ts` resolves the organization named by `input.slug` and proves the caller is a member before the handler runs - handlers get `context.organization` and `context.membership`.
...
Then declare each table through `pgTable.withRLS` again (see the note in `packages/db/src/drizzle-schema-auth.ts`) and run `pnpm db:push`.

## Security Notes

- Validate all inputs with Zod at oRPC boundaries
- Authorization lives in oRPC procedures, not the database - always check org membership in routers
...
2. **API development** - Build [oRPC procedures](/docs/architecture/api)
