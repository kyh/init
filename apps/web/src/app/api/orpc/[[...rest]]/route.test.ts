...
/**
 * The CSRF plugin is this endpoint's only cross-site defense — auth cookies
 * are SameSite=None — and it lives in transport config that typecheck cannot
 * see. Driving the real exported handlers pins the wiring: removing the
 * plugin, adding CORS headers, or exporting OPTIONS turns CI red instead of
 * silently shipping an unguarded endpoint.
 */

const post = (headers: Record<string, string>) =>
  route.POST(
    new NextRequest("http://localhost:3000/api/orpc/todo/list", {
      method: "POST",
